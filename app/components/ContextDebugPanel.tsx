...
export function ContextDebugPanel({
  ...
  return (
    <div className="settings-group">
      ...
      >
        上下文调试
        <span style={{ fontSize: "12px", color: "#a09088", transform: open ? "rotate(90deg)" : "none", transition: "transform 0.2s" }}>›</span>
      </button>
      {open && (
        <div style={{ fontSize: "13px", color: "#6b5b53", lineHeight: 2 }}>
          ...
          <div>长期记忆:<span style={{ marginLeft: "8px", color: cache?.summer_used ? "#5b8a6b" : "#b5aca6" }}>{cache?.summer_used ? "summer" : "等待下一轮确认"}</span></div>
          {cache ? (
            <>
              ...
              <div>是否截断:<span style={{ marginLeft: "8px", color: cache.context_truncated ? "#c4866c" : "#5b8a6b" }}>
              ...
              <div>旧摘要/旧记忆:<span style={{ marginLeft: "8px", color: "#b5aca6" }}>不再注入</span></div>
            </>
          ) : (
