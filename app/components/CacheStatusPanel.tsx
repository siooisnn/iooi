...
function getCacheStatusColor(cache: CacheStats) {
  if (cache.status === "hit") return "var(--theme-success, #5b8a6b)";
    ...
    <div className="settings-group">
      ...
      {cache ? (
        <div style={{ fontSize: "13px", color: "var(--theme-secondary, #6b5b53)", lineHeight: 2 }}>
          {cache.model && <div>实际模型:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>{cache.model}</span></div>}
          {cache.backend && <div>通道:<span style={{ marginLeft: "8px", color: cache.backend === "claude-code" ? "var(--theme-success, #5b8a6b)" : "var(--theme-muted, #8a7d75)" }}>{cache.backend === "claude-code" ? "Claude 订阅" : "API"}</span></div>}
          <div>状态:<span style={{ marginLeft: "8px", color: getCacheStatusColor(cache) }}>{getCacheStatusLabel(cache)}</span></div>
          {cache.reason && <div>说明:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>{cache.reason}</span></div>}
          ...
          <div>输入总 token:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>{totalInputTokens || cache.prompt_tokens || "-"}</span></div>
          {cache.time && <div>上次回复:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>{cache.time}</span></div>}
          {typeof cache.total_ms === "number" && (
            <div>上轮耗时:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>
              总计 {durationLabel(cache.total_ms)} · 模型 {durationLabel(cache.claude_duration_ms ?? 0)}
            </span></div>
          )}
          {typeof cache.total_ms === "number" && (
            <div>耗时分段:<span style={{ marginLeft: "8px", color: "var(--theme-muted, #8a7d75)" }}>
          ...
          <div>读取缓存:<span style={{ marginLeft: "8px", color: (cache.cache_read ?? 0) > 0 ? "var(--theme-success, #5b8a6b)" : "var(--text-light)" }}>{cache.cache_read ?? 0}</span></div>
          ...
          {(cache.cache_read ?? 0) > 0 && totalInputTokens > 0 && (
            <div style={{ marginTop: "4px", color: "var(--accent-text)" }}>
              命中率约 {hitRate}%
              <div style={{ color: "var(--theme-muted, #8a7d75)" }}>读取/总输入: {cache.cache_read ?? 0}/{totalInputTokens}</div>
            </div>
          )}
